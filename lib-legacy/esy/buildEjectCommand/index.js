...
function buildEjectCommand(sandbox) {
  ...
  let sandboxPath = function sandboxPath(packageInfo, tree) {
    ...
    if (isRootPackage) {
      ...
      return ['$ESY__LOCAL_STORE', tree, packageKey].concat(path).join('/');
    }
  ...
  traversePackageDependencyTree(sandbox.packageInfo, packageInfo => {
    ...
    function definePackageRule(rule) {
      ...
      rules.push({
        ...
        dependencies: ['esy-store', 'esy-local-store', 'esy-root'].concat((0, (_toConsumableArray2 || _load_toConsumableArray()).default)(dependencies)),
        phony: true,
        command: [outdent`
